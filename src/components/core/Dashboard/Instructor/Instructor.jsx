    ...
    { loading? (
      ...
      <div className='rounded-md bg-richblack-800 p-6'>
        <div className='flex items-center justify=between'>
            <p className='text-lg font-bold text-richblack-5 px-3'>Your courses </p>
            <Link to="/dashboard/my-courses">
            <p className='text-sm font-semibold text-yellow-50'>{" "}View All</p></Link>

        </div>
        <div className='my-4 flex items-start space-x-6'> 
            {courses.slice(0,3).map((course)=>(
                <div key={course._id} className='w-1/3'>
                    <img src={course.thumbnail} alt={course.coureName}
                    className='h-[200px] w-full rounded-md object-cover' />
                    <div className='mt-3 w-full'>
                        <p className='text-sm font-medium text-richblack-50'>{course.courseName}</p>
                        <div className='mt-1 flex items-center space-x-2'>
                            <p className='text-xs font-medium text-richblack-300'>
                                {course.studentEnrolled.length} Student(s)
                            </p>
     ...
     <Link to='/dashboard/add-course'><p className='mt-1 text-center text-lg font-semibold text-yellow-50'>
        Create a courses</p></Link>
     </div>
    )}
 </div>
